...
| 2026-07-31 | Consent prompt is a designed card on BOTH hosts, drawn natively on Windows | The prompt the user actually sees on a TransportMode host was a stock `MessageBoxW`, which cannot render a brand mark, accent colour, device-id block, copy button or checkbox. It is now hand-drawn in `agent/session/consentwin_windows.go` (GDI, owner-drawn, DPI-scaled) and mirrored by `consent_dialog.dart` for macOS/attended hosts, so one design ships everywhere. The native window uses Segoe UI / Consolas: the bundled Space Grotesk / JetBrains Mono are Flutter assets the Go worker cannot load. |
| 2026-07-31 | "Remember this decision" is wired, not decorative | Shipping the checkbox unwired would have broken the Data Honesty Rule. It persists per device id and applies in BOTH directions — a remembered Decline auto-declines. Stores are deliberately separate per host mode (`consent-decisions.json` for the Go worker, SharedPreferences for Flutter) because a decision made in one mode should not silently apply in the other. |
...
| 2026-07-24 | PIVOT: revert Obsidian (r83-r88), restore v3 LIGHT theme (r89); target new user light dashboard mockup | User reviewed a light dashboard mockup, chose "switch fully to this light design" + "omit invented tiles". Obsidian dark shelved. |
