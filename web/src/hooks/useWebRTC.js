...
export function useWebRTC({ send, on, peerID, iceServers = ICE_SERVERS, getLocalIPs }) {
  ...
  const start = useCallback(async (targetID) => {
    ...
    dc.onmessage = async (e) => {
      try {
        ...
        if (msg.type === 'displays_info') {
          ...
          console.log('[webrtc] cursor update', { x: msg.x, y: msg.y, visible: msg.visible })
          setCursorInfo(msg)
    ...
    // This ensures the agent receives a complete offer with all local candidates
    await new Promise(resolve => {
      if (pc.iceGatheringState === 'complete') {
        ...
        // Also check periodically in case gathering completes before we set up handler
    ...
    send({ type: 'offer', to: targetID, payload: { sdp: pc.localDescription.sdp, type: pc.localDescription.type } })
  ...
  return { start, stop, stream, connectionState, displays, cursorInfo, sendInput, sendClipboard, sendChat, sendFile }
}
